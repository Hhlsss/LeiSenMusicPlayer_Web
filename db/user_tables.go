...
// CreateOrUpdateUserProfileByUUID 使用UUID创建或更新用户个人资料
func CreateOrUpdateUserProfileByUUID(userUUID, email, nickname string) error {
	if client == nil {
		if err := Init(); err != nil {
			return err
		}
	...
	case http.StatusNotFound:
		...
		// 重试操作
		return CreateOrUpdateUserProfileByUUID(userUUID, email, nickname)
	default:
		// 读取错误响应体获取更多信息
